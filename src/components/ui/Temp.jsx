...
const validateForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    setFieldErrors({}); // Clear specific field errors
    ...
    try {
      if (mode === 'login') {
        ...
        if (!success) {
          onAuthError('Invalid email or password. Please try again.', 'error');
        }
      } else {
        const payload = {
        ...
        onAuthSuccess(responseData.message || 'Registration successful!');
      ...
      if (error.status === 400 && error.data) {
        // ... existing backend error handling ...
        if (error.data.detail) {
          onAuthError(error.data.detail, 'error');
        } else if (Object.keys(error.data).length > 0) {
          onAuthError('Please correct the errors in the form.', 'error');
        } else {
          onAuthError('Registration failed. An unknown error occurred.', 'error');
        }
      } else {
        onAuthError(
